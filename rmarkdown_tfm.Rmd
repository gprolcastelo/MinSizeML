...
minimum_sample_knn(X = X_market, 
                   Y = Y_market,
                   p_vec = 1:99/100,
                   param = "Accuracy",  # "Accuracy" or "Kappa"
                   thr_param = 0.9,
                   n.cores = 1)
)
```
...
```{r}
# Data:
wbcd_n <- read.csv("./data/classification/wbcd_normalized.csv",
                   header = T,
                   stringsAsFactors = T)
# head(wbcd_n)
# Variable to predict:
...
```{r}
plot(ameth_rf$training_set_size,ameth_rf$R2_vec)
fit_R2 <- lm(ameth_rf$R2_vec ~ ameth_rf$training_set_size)
abline(fit_R2)
```


```{r}
summary(lm(ameth_rf$R2_vec ~ ameth_rf$training_set_size))
```
